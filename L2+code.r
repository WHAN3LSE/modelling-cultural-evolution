...
#create shared knowledge variable (if 2 people don't know then that means shared ignorance)
#If both knows (1,1) then this is shared knowledge)
m.dyads$shared <- ifelse(m.dyads$know1 == 1 & m.dyads$know2 == 1, 1, 0)

#Chi-Squared Test
table(m.dyads$shared,m.dyads$dyadsex)

test <- chisq.test(x = table(m.dyads$shared,
                                        m.dyads$dyadsex))

test

stest <- chisq.test(x = table(m.dyads$shared,
                                        m.dyads$samesex))
stest